...
{
    /// <summary>
    /// Displays a relevant portion of the game world to the screen.
    /// Follows an entity.
    /// </summary>
    public class Camera2D
    {
        ...
        {
            _viewport = viewport;
            _projection = Matrix.CreateOrthographicOffCenter(0f, ConvertUnits.ToSimUnits(_viewport.Width),
                                                             ConvertUnits.ToSimUnits(_viewport.Height), 0f, 0f,
                                                             1f);
            _view = Matrix.Identity;
            _batchView = Matrix.Identity;

            _translateCenter = new Vector2(ConvertUnits.ToSimUnits(_viewport.Width * 0.5f),
                                           ConvertUnits.ToSimUnits(_viewport.Height * 0.5f));
        ...
        public Matrix ParallaxView
        {
            get
            {
                // To add parallax, simply multiply it by the position and center (origin)
                ...
                translateCenter = ConvertUnits.ToDisplayUnits(translateCenter * Parallax);
                translateBody = ConvertUnits.ToDisplayUnits(translateBody * Parallax);

                return Matrix.CreateTranslation(translateBody) *
                        matRotation *
                        matZoom *
        ...
        public Vector2 ParallaxPosition
        {
            get
            {
                Vector2 parallaxOffset = new Vector2(32, 18);
                float parallaxMult = (1.0f - Parallax) * 10f;
                parallaxOffset *= parallaxMult;

                return ConvertUnits.ToDisplayUnits(_currentPosition * Parallax) + parallaxOffset;
            }
        }
        ...
        public void Update(float dt)
        {
            ...
            _currentPosition.X += scrollX;
            _currentPosition.Y += scrollY;
            
            _currentRotation += 80f * rotDelta * rotInertia * dt;
